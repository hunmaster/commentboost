...
# 공용제품 동반 셔플 — 범용 제품과 브랜드를 함께 언급해 자연스러움↑ (일부 댓글에만)
COMPANION_PRODUCTS = [
    "세타필", "시중 보습제", "병원 처방 보습제", "바세린", "약국 연고",
    "수딩젤", "저자극 로션", "아토피 크림",
]
COMPANION_PRODUCT_PROB = 0.4  # 이 확률로만 공용제품 동반 언급(전부 X)
...
def generate(keyword, title, description, url, brand=None, api_key=None, model=None, max_comment_attempts=2):
    ...
    try:
        ...
        # 공용제품 동반 언급(확률) — 범용 제품 + 브랜드 병행 후기 톤
        companion_instruction = ""
        if random.random() < COMPANION_PRODUCT_PROB:
            cp = _pick(COMPANION_PRODUCTS)
            companion_instruction = (
                f"[공용제품 동반] 이번 댓글에는 '{cp}' 같은 많이 쓰는 범용 제품을 자연스럽게 함께 언급하세요. "
                f"예: \"{cp} 바르기 전에 {brand} 먼저 뿌리고 쓰니까 덜 가렵고 보습도 더 잘 잡히더라구요\"처럼 "
                f"{brand}와 '병행해서 쓰는' 실사용 후기 톤으로. 광고처럼 들리지 않게 딱 한 번만, 억지로 끼워넣지 말 것."
            )
        ...
        comment_user = _render(_PROMPTS["comment_user"], ctx)
        if companion_instruction:
            comment_user += "\n\n" + companion_instruction
        for _ in range(max(1, max_comment_attempts)):
            comment_text = _chat(_render(_PROMPTS["comment_system"], ctx), comment_user, 0.95, 700, top_p=0.95, api_key=api_key, model=model)
